...
router.post('/stream', async (req, res) => {
    ...
    try {
        ...
        if (web_search && message && String(message).trim() && !shouldUseRuntimeDate && shouldAllowWebSearch(message)) {
            const explicitWebSearchIntent = hasExplicitWebSearchIntent(message);
            if (!explicitWebSearchIntent) {
                res.write(`data: ${JSON.stringify({
                    type: 'status',
                    status: 'search_skipped',
                    message: '当前问题没有明确要求联网检索，已跳过以避免资料污染。',
                    results: [],
                })}\n\n`);
            } else {
                res.write(`data: ${JSON.stringify({
                ...
                try {
                    ...
                    if (!searchQuery || !isReliableSearchQuery(message, searchQuery)) {
                        res.write(`data: ${JSON.stringify({
                            type: 'status',
                            status: 'search_skipped',
